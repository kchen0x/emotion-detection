...
def featureExtractionWithFixedSizeDirWrapper(directory,mtSize, stWin, stStep):
    if not os.path.isdir(directory):
        raise Exception("Input path not found!")
    aF.mtFeatureExtractionWithFixedSizeToFileDir(directory, mtSize, stWin,
                                    stStep, True, True, True)
...
def parse_arguments():
    ...
    featExtFixedDir = tasks.add_parser("featureExtractionWithFixedSizeDir",
                               help="Extract audio features with fixed size from file")
    featExtFixedDir.add_argument("-i", "--input", required=True,
                            help="Input directory")
    featExtFixedDir.add_argument("-sw", "--stwin", type=float, default=0.050,
                            help="Short-term window size")
    featExtFixedDir.add_argument("-ss", "--ststep", type=float, default=0.050,
                            help="Short-term window step")
    featExtFixedDir.add_argument("-ms", "--mtsize", type=int, required=True,
                            help="Mid-term feature size")
...
if __name__ == "__main__":
    ...
    elif args.task == "featureExtractionWithFixedSizeDir":
        # Feature extraction for all WAV files stored in a folder where result size is fixed
        # EXAMPLE:python audioAnalysis.py featureExtractionWithFixedSizeDir -i emotion/data_wav -ms 5 -sw 0.050 -ss 0.050
        # param1:path for put out 
        # param2:middle-term size
        # param3:short-term windows size
        # param4:short-term step 
        featureExtractionWithFixedSizeDirWrapper(args.input, args.mtsize,
                                    args.stwin, args.ststep)
