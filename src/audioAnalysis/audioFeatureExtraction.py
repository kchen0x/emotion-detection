...
#For different size of short-term features,ouptput the same size mtSize of Mid-term
def mtFeatureExtractionWithFixedSize(signal, Fs,  mtSize, stWin, stStep):
    """
    ...
    for i in range(numOfFeatures):        # for each of the short-term features:
        ...
        N = len(stFeatures[i])
        mtWinRatio = N/mtSize
        mtStepRatio = N/mtSize
        while (N-curPos >= mtWinRatio):
            N1 = curPos
...
def mtFeatureExtractionWithFixedSizeToFile(fileName, mtSize, shortTermSize, shortTermStep, outPutFile,
                              storeStFeatures=False, storeToCSV=False, PLOT=False):
    """
    This function is used as a wrapper to:
    a) read the content of a WAV file
    ...
    if storeStFeatures:
        ...
        [mtF, _] = mtFeatureExtractionWithFixedSize(x, Fs, mtSize, round(Fs * shortTermSize), round(Fs * shortTermStep))
...
def mtFeatureExtractionWithFixedSizeToFileDir(dirName, mtSize, shortTermSize, shortTermStep, storeStFeatures=False, storeToCSV=False, PLOT=False):
    types = (dirName + os.sep + '*.wav', )
    filesToProcess = []
    for files in types:
        filesToProcess.extend(glob.glob(files))
